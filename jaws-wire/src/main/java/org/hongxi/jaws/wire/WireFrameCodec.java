 ...
 * </pre>
 * The compressed flag is {@code 0} for uncompressed (the only mode supported
 * by jaws-wire) and {@code 1} when a compression encoding is applied.
 * <p>
...
public final class WireFrameCodec {
    ...
    /**
     * Encode a protobuf {@link Message} into a gRPC frame:
    ...
    public static ByteBuf encode(Message message, ByteBufAllocator alloc) {
        ...
        ByteBuf buf = alloc.buffer(WireConstants.GRPC_HEADER_SIZE + data.length);
        buf.writeByte(WireConstants.NOT_COMPRESSED);
    ...
    public static <T extends Message> T decode(ByteBuf frame, Parser<T> parser)
        ...
        if (compressedFlag != WireConstants.NOT_COMPRESSED) {
            throw new InvalidProtocolBufferException(
                    "Compressed gRPC frames are not supported; flag=" + compressedFlag);
        }
    ...
    /**
     * Encode raw protobuf bytes into a gRPC frame:
     * {@code [compressed-flag(1)][length(4)][protobuf bytes]}.
     *
    ...
    public static ByteBuf encodeRawBytes(byte[] rawBytes, ByteBufAllocator alloc) {
        ByteBuf buf = alloc.buffer(WireConstants.GRPC_HEADER_SIZE + rawBytes.length);
        buf.writeByte(WireConstants.NOT_COMPRESSED);
        buf.writeInt(rawBytes.length);
