...
已复核确认无问题的部分：`DefaultResponseFuture` 状态机（wait/notify + volatile state）、`NettyChannel`（volatile 快照 + 锁内状态迁移）、`AbstractTransportFactory`（锁一致）、`ConnectionLimitHandler`、`FailbackCluster` DCL、`DefaultRequest` 的 id 生成（AtomicLong 自增）、`EagerThreadPoolExecutor`、`NacosRegistry`（lock/unlock 一致）、`MetricsFilter`/`TracingFilter` 等。
