...
| **流式调用** | 不支持 | 支持 Stream/Streaming 模式 | **差距大** |
...
| 能力维度 | Jaws | Dubbo | 差距评估 |
|---------|------|-------|---------|
| **负载均衡** | random / roundRobin / leastActive / shortestResponse / consistentHash（5 种） | 同左 + p2c / adaptive 等（10+ 种） | 小幅差距 |
| **高可用容错** | failover / failfast / failback（3 种） | failover / failfast / failback / forking / available（5 种） | 小幅差距 |
...
| **Triple/gRPC** | 不支持 | 支持 Triple（兼容 gRPC） | **差距大** |
| **REST/HTTP** | REST 桥接（Servlet） | 原生 rest 协议（JAX-RS） | **差距较大** |
| **Injvm** | 支持 | 支持 | **持平** |
| **多协议同时暴露** | 不支持 | 支持同一服务多协议暴露 | **差距较大** |
...
| **代码生成** | 不支持 | 支持 IDL 代码生成（protobuf/thrift） | 中等差距 |
...
3. **多协议支持** — 缺少 Triple/gRPC 协议，不支持多协议同时暴露，跨语言互通能力弱
...
Jaws 在**核心 RPC 链路**（协议、编解码、零拷贝、心跳、优雅停机、异步调用）上已与 Dubbo **基本持平**，在**标签路由/灰度发布**和**多注册中心**能力上已补齐。但在**生态广度**（序列化种类、注册中心种类、协议种类、运维工具）和**流量治理深度**（限流熔断）上差距较大。这些差距本质上是 Dubbo 多年社区积累的结果。
