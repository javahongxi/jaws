...
- **变更放大**：ZK 消费者 watch 服务父节点，任一子节点变化即触发 watch 后**重新拉全量**；消费者 × 服务数一大就是惊群 + O(N) 回捞。Nacos 定向/增量推送最新列表（见 §三）。
- **一致性换扩展上限——ZK 的被低估优点**：CP 下节点下线经共识提交后才让 watch 生效，消费者几乎不会拿到"已死实例"的窗口；Nacos 的 AP 路径在收敛前可能短暂推到一个刚挂的实例。中小规模，这笔"用一致性换扩展上限"的交换很划算。

**取舍结论**：中小规模选 ZK 完全够用，且存活语义更严格；规模一大则 Nacos 的 AP 写 + 定向推 + namespace/权重/健康面板成套治理是硬需求。一个现实前提——**只在"本来就在运维 ZK"（Kafka/Hadoop/Dubbo 生态）时复用 ZK 当注册中心最划算**；若没有，单为注册去维护一套 ZK，不如直接上功能更全的 Nacos。jaws 两腿都实现，正是让你按"已有什么 + 要多强存活一致性 + 规模多大"来选，而不被框架绑定。
