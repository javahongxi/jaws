...
Jaws 是一个**核心 3 万余行、可以从头读到尾**的轻量级 RPC 框架。它用约 Dubbo 核心骨架 1/7 的代码量，完整实现了一个工业级 RPC 的核心机制，支持 Netty TCP 与 HTTP/2 多传输、gRPC 线格式对等互通、三种流式调用、自适应负载均衡与高可用容错，实测约 140K QPS。目标是成为 **RPC 骨架的标杆**——读完 Jaws 源码，再去读 Dubbo 会快很多。
