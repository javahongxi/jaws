...
import org.hongxi.summer.protocol.summer.GrowableByteBuffer;

import java.io.IOException;
import java.nio.ByteBuffer;
import java.util.*;

import static org.hongxi.summer.serialize.SimpleSerialization.SimpleType.*;
...
public class SimpleSerialization implements Serialization {
    ...
    private void serialize(Object obj, GrowableByteBuffer buffer) throws IOException {
        ...
        throw new SummerServiceException("SimpleSerialization unsupported type: " + clz);
    }
    ...
    private <T> T deserialize(GrowableByteBuffer buffer, Class<T> clz) throws IOException {
        ...
        throw new SummerServiceException("SimpleSerialization not support " + type + " with receiver type:" + clz);
    }
    ...
    private int getAndCheckSize(GrowableByteBuffer buffer) {
        ...
        if (size > buffer.remaining()) {
            throw new SummerServiceException("SimpleSerialization deserialize fail! buffer not enough!need size:" + size);
        }
        return size;
    }
    ...
    private Map<String, String> readStringMap(GrowableByteBuffer buffer) throws IOException {
        ...
        if (buffer.position() != endPos) {
            throw new SummerServiceException("SimpleSerialization deserialize wrong map size, except: " + size + " actual: " + (buffer.position() - startPos));
        }
        return map;
    }
    ...
    private <T extends Collection> T readStringCollection(GrowableByteBuffer buffer, T collection) throws IOException {
        ...
        if (buffer.position() != endPos) {
            throw new SummerServiceException("SimpleSerialization deserialize wrong array size, except: " + size + " actual: " + (buffer.position() - startPos));
        }
        return collection;
    }
